...
//                     axios.get('http://localhost:5000/api/cabDetails', headers),
//                     axios.get('http://localhost:5000/api/assigncab', headers),
//                     axios.get('http://localhost:5000/api/cabs/cabExpensive', headers)
//                 ]);

//                 // ✅ Handle possible errors
//                 if (driversRes.status === "rejected") console.warn("Failed to fetch drivers:", driversRes.reason);
//                 if (cabsRes.status === "rejected") console.warn("Failed to fetch cabs:", cabsRes.reason);
//                 if (assignedCabsRes.status === "rejected") console.warn("Failed to fetch assigned cabs:", assignedCabsRes.reason);
//                 if (expensesRes.status === "rejected") console.warn("Failed to fetch expenses:", expensesRes.reason);
...
//                 const monthlyExpenseData = expensesData.length > 0 ? expensesData.map((exp, index) => ({
//                     month: `Cab ${index + 1} (${exp.cabNumber || 'Unknown'})`,
//                     expense: exp.totalExpense || 0
//                 })) : [{ month: "No Data", expense: 0 }];
...
//                 const formattedBreakdown = Object.keys(aggregatedBreakdown).length > 0
//                     ? Object.keys(aggregatedBreakdown).map(key => ({ name: key, value: aggregatedBreakdown[key] }))
...
//                             <motion.div className="bg-gray-800 p-6 shadow-xl rounded-lg">
    ...
    useEffect(() => {
        const fetchDashboardData = async () => {
            try {
                ...
                // Use correct breakdown keys based on your data: Fuel, FastTag, Tyre, Other
                const aggregatedBreakdown = expensesData.reduce((acc, exp) => {
                    const breakdown = exp.breakdown || {};
                    acc.fuel = (acc.fuel || 0) + (breakdown.fuel || 0);
                    acc.fasttag = (acc.fasttag || 0) + (breakdown.fastTag || breakdown.fasttag || 0); // 🔥 Fix here
                    ...
                    <>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                            ...
                            <motion.div className="bg-gray-800 p-6 shadow-xl rounded-lg">
                                ...
                                <ResponsiveContainer width="100%" height={300}>
                                    <PieChart>
                                        <Pie data={expenseBreakdown} dataKey="value" nameKey="name" label>
                                            {expenseBreakdown.map((entry, index) => (
                                                <Cell key={index} fill={COLORS[index % COLORS.length]} />
                                            ))}
                                        </Pie>
