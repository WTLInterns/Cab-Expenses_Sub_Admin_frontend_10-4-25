...
//         const [driversRes, cabsRes] = await Promise.all([
//           axios.get(`${API_BASE_URL}/driver/profile`),
//           axios.get(`${API_BASE_URL}/cabDetails`),
//         ])
...
//       setMessage(`❌ ${error.response?.data?.message || "Failed to assign cab"}`)
//     } finally {
//       setLoading(false)
//       setTimeout(() => setMessage(""), 3000)
//     }
...
//       formData.append("cabNumber", cabFormData.cabNumber);
//       formData.append("insuranceNumber", cabFormData.insuranceNumber);
//       formData.append("insuranceExpiry", cabFormData.insuranceExpiry);
//       formData.append("registrationNumber", cabFormData.registrationNumber);
//       formData.append("addedBy", cabFormData.addedBy);
//       if (cabFormData.cabImage) {
//         formData.append("cabImage", cabFormData.cabImage);
//       }
  
//       const response = await axios.post(
//         "http://localhost:5000/api/cabDetails/add",
//         formData,
//         {
//           headers: { 
//             Authorization: `Bearer ${token}`,
...
//       // SUCCESS: Refresh the cabs list
//       const refreshedCabs = await axios.get("http://localhost:5000/api/cabDetails", {
//         headers: { Authorization: `Bearer ${token}` }
//       });
      
//       setCabs(refreshedCabs.data); // Update state with fresh data
//       setCabFormSuccess("✅ Cab added successfully!");
...
//     } catch (error) {
//       console.error("Add cab error:", error.response?.data || error);
//       setMessage(`❌ ${error.response?.data?.message || "Failed to add cab"}`);
//     } finally {
//       setLoading(false);
//     }
//   };
...
//                     <option value="">Select Cab</option>
//                     {cabs.map((cab) => (
//                       <option key={cab._id} value={cab.cabNumber || cab._id}>
//                         {cab.cabNumber} - {cab.model}
//                       </option>
//                     ))}
//                   </select>
...
//               <button
//                 onClick={handleAssign}
//                 disabled={!selectedDriver || !selectedCab}
//                 className={`w-full px-4 py-3 text-white rounded-lg transition-colors duration-300 font-medium ${
