...
const AssignCab = () => {
    ...
    const handleAddCabSubmit = async (e) => {
        ...
        try {
            ...
            Object.keys(cabFormData).forEach((key) => {
                formDataToSend.append(key, cabFormData[key]);
            });
            ...
            if (response.ok) {
                ...
                setCabFormErrors({ apiError: typeof data.error === "string" ? data.error : "Something went wrong" });
            }
        } catch (error) {
    ...
    return (
        ...
        >
            ...
            <div className="flex-1 p-4 md:p-6 md:mt-46 mt-20 sm:mt-0  transition-all duration-300">
                <motion.div 
                            ...
                            {message && (
                                <motion.p 
                                    className={`mt-4 text-center font-medium text-sm md:text-base ${
                                        ...
                                        message.startsWith("⚠️") ? "text-yellow-400" : "text-red-400"
                                    }`}
                                    initial={{ opacity: 0 }}
                                    animate={{ opacity: 1 }}
                                >
